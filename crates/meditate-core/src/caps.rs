//! Terminal capability data and detection, expressed against an injected
//! environment so it carries no `std::env` dependency and compiles to wasm.
//!
//! The native binding that reads the real process environment (`SystemEnv`)
//! lives in the `meditate` crate; everything here operates on the [`Env`] trait,
//! so the detection logic is unit-testable — and wasm-buildable — without
...
/// without mutating the real process environment — and so the core stays free
/// of `std::env` for the wasm build (the real binding lives in the CLI).
pub trait Env {
...
/// An in-memory environment for tests and for callers (like the web build) that
/// describe their capabilities explicitly rather than reading a real shell.
#[derive(Default)]
